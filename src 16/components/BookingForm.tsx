          ...
          <CardContent className="text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
              ...
              {values.bookingDate && values.bookingTime && <p className="text-muted-foreground">We'll see you on {format(values.bookingDate, 'PPP')} at {values.bookingTime}.</p>}
               {values.deliveryMethod === 'dropoff' && (
                <div className="mt-4 text-muted-foreground bg-secondary/30 p-4 rounded-lg">
                    <p className="font-semibold text-foreground">Next Steps:</p>
                    <p>Please hand your sneakers to reception, quoting "For Sneakswash" at:</p>
                    <a 
                        href="https://www.google.com/maps/search/?api=1&query=Store+first+St+Modwen+Rd%2C+Trafford+Park%2C+M32+0ZF" 
                        target="_blank" 
                        rel="noopener noreferrer" 
                        className="font-semibold text-primary mt-1 block hover:underline"
                    >
