...
export const AdminMobileView = (props: any) => {
  ...
  const stats = useMemo(() => {
    ...
    const totalRevenue = receivedOrdersThisMonth.reduce(
      (acc: number, order: Order) => acc + (order.totalCost || 0),
      0
    );
    ...
    <main className="flex-1 p-4 space-y-6">
      ...
      <section>
        <Card className="bg-card/80 backdrop-blur-sm">
          ...
          <CardContent className="max-h-[500px] overflow-y-auto styled-scrollbar space-y-3 p-3">
            {isLoading ? (
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {order.service} (x{order.quantity || 1})
                        </p>
                      </div>
                      <Badge
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto styled-scrollbar p-1">
              <h3 className="font-semibold mb-2">
                Slots for {selectedDate ? format(selectedDate, "PPP") : "..."}
              </h3>
        ...
        <Card className="bg-card/80 backdrop-blur-sm">
          ...
          <CardContent>
            <Tabs
              ...
              onValueChange={(value) => setProjectionPeriod(value as any)}
              className="w-full"
            >
