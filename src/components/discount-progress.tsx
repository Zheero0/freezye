...
export default function DiscountProgress() {
  ...
  const itemsAway =
    (itemsNeededForOffer - (itemCount % itemsNeededForOffer)) %
    itemsNeededForOffer;
  const progress =
    ((itemsNeededForOffer - itemsAway) / itemsNeededForOffer) * 100;
    ...
    <div className="bg-muted p-4 rounded-lg text-center">
      <div className="flex items-center justify-center gap-2 mb-2">
        <Gift
          className={cn(
            "h-5 w-5",
            isOfferUnlocked ? "text-primary" : "text-muted-foreground"
          )}
        />
