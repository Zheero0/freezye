...
export default function OrderManagement() {
  ...
  const StatusSelector = ({ order }: { order: Order }) => (
    ...
    >
        <SelectTrigger className={cn("w-32 focus:ring-0 focus:ring-offset-0 border-none !bg-transparent", {
  ...
  const MobileView = () => (
    <div className="space-y-4">
      {orders.length > 0 ? (
        orders.map((order) => (
          <Link href={`/admin/orders/${order.id}`} key={order.id} className="block">
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>View and manage simulated orders. Data is stored in your browser's local storage.</CardDescription>
      </CardHeader>
      <CardContent>
        {!isClient ? <LoadingSkeleton /> : (isMobile ? <MobileView /> : <DesktopView />)}
      </CardContent>
    </Card>
  );
}
